...
const UTBKStudentApp = () => {
  ...
  const AnalysisDashboard = () => {
      ...
      else motivation = "Jangan menyerah, banyak latihan lagi.";
      ...
      if (avgTimePerQuestion < 45) {
          speedAnalysis = totalScore < 500 ? "Terburu-buru" : "Sangat Efisien";
      } else if (avgTimePerQuestion > 180) {
          speedAnalysis = "Terlalu Lama (Stuck)";
      }
      ...
      return (
          <div className="space-y-6 text-left">
              <div className="bg-white border border-gray-200 p-6 rounded-xl text-center shadow-sm">
                  ...
                  <div className="text-6xl font-extrabold text-indigo-600 mb-2">{totalScore}</div>
                  ...
                  <div className="grid grid-cols-3 gap-4 border-t pt-4">
                      <div>
                          ...
                          <span className="font-bold text-indigo-600">{tpsPercent > litPercent ? 'TPS' : 'LITERASI'}</span>
                      </div>
                      <div>
                          <span className="block text-xs text-gray-400 font-bold uppercase">Tempo</span>
                          <span className="font-bold text-indigo-600">{speedAnalysis}</span>
                      </div>
                      <div>
                          <span className="block text-xs text-gray-400 font-bold uppercase">Sisa Waktu</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {SUBTESTS.map(s => {
                      ...
                      return (
                          <div key={s.id} className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm flex flex-col justify-between">
                              <div className="flex justify-between items-start mb-2">
                                  <div>
                                      ...
                                      <h4 className="font-bold text-gray-700 mt-1 text-sm">{s.name}</h4>
                                  </div>
                              ...
                              <div>
                                  ...
                                  <div className="w-full bg-gray-100 rounded-full h-2">
                                      <div className={`h-2 rounded-full ${score < 0 ? 'bg-red-500' : 'bg-indigo-500'}`} style={{ width: `${Math.max(5, (score/250)*100)}%` }}></div>
                                  </div>
                              </div>
                          </div>
                      )
