...
import { Clock, Ticket, AlertCircle, CheckCircle, XCircle, ShieldAlert, Timer, Trophy, Copyright, CheckSquare, AlignLeft, List, Activity, ChevronDown, ChevronUp } from 'lucide-react';
...
const UTBKStudentApp = () => {
  ...
  // --- ANALISIS DASHBOARD (LOGIKA WAKTU DIPERBAIKI) ---
      ...
      return (
          <div className="space-y-6 text-left">
              <div className="bg-indigo-50 border border-indigo-200 p-6 rounded-xl text-center shadow-sm">
                  ...
                  <div className="text-5xl font-extrabold text-indigo-700 mb-2">{totalScore}</div>
                  <p className="text-indigo-600 font-medium">{motivation}</p>
                  
                  <div className="mt-4 flex justify-center gap-4 text-sm text-gray-600">
                      <span className="bg-white px-3 py-1 rounded border border-gray-200 flex items-center gap-2">
                         <Activity size={14}/> Dominasi: <strong>{tpsPercent > litPercent ? 'TPS' : 'LITERASI'}</strong>
                      </span>
                      {/* INI BAGIAN YANG DIUBAH: MENAMPILKAN SISA WAKTU GLOBAL */}
                      <span className="bg-white px-3 py-1 rounded border border-gray-200 flex items-center gap-2">
                         <Clock size={14}/> Sisa Waktu Global: <strong>{formatTime(timeLeft)}</strong>
                      </span>
                  </div>
              </div>
                      ...
                      return (
                          <div key={s.id} className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm flex flex-col justify-between hover:bg-gray-50 transition">
                              <div className="flex justify-between items-start mb-2">
                                  <div>
                                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${isTPS ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                                          {isTPS ? 'TPS' : 'LITERASI'}
                                      </span>
