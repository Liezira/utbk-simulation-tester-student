...
// --- ADVANCED SECURITY COMPONENT (Full Features + SP1 Logic) ---
const AdvancedSecurityMonitor = ({ 
  ...
  tokenCode, 
  onForceSubmit,
  onWarning // Prop untuk trigger SP1
}) => {
  const [violations, setViolations] = useState({
    ...
    rightClick: 0,
    screenshot: 0,
    systemKey: 0 // Tambahan untuk tombol Windows
  });
  ...
  const handleViolation = async (type, message) => {
    setViolations(prev => {
      ...
      // Trigger callback ke parent (Main App)
      ...
      else if (totalViolations >= SECURITY_CONFIG.MAX_VIOLATIONS) {
        // Pelanggaran Kedua -> Auto Submit
        onForceSubmit(`DISKUALIFIKASI: Melanggar aturan keamanan sebanyak ${totalViolations} kali.`);
      }
  ...
  const logViolationToFirebase = async (type, message, totalCount) => {
    try {
      ...
      // Menggunakan updateDoc tanpa await agar tidak blocking UI
      updateDoc(docRef, {
        [`violations.${type}`]: totalCount,
        lastViolation: { type, message, timestamp: new Date().toISOString() }
      }).catch(err => console.error("Log error", err));
    } catch (error) { console.error('Failed to log violation:', error); }
  };

  // --- 1. CSS INJECTION UNTUK MENCEGAH SELEKSI (UI OBFUSCATION) ---
  useEffect(() => {
    if (!isActive) return;
    // Mencegah seleksi teks (user tidak bisa blok soal)
  ...
  // --- 2. DETEKSI SCREENSHOT & LAYAR (INTERVAL) ---
  useEffect(() => {
    if (!isActive) return;
    ...
    const detectScreenAnomalies = () => {
      ...
      // Logika: Jika document hidden sangat cepat (<100ms) lalu aktif lagi, biasanya screenshot tool
  ...
  // --- 3. DETEKSI DEVTOOLS (UKURAN WINDOW) ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // A. Blur / Pindah App
    const handleBlur = () => {
      handleViolation('blur', '⚠️ Dilarang berpindah aplikasi / membuka menu lain!');
    };

    // B. Visibility (Minimize / Pindah Tab)
    const handleVisibilityChange = () => {
      if (document.hidden) {
        handleViolation('visibility', '⚠️ Terdeteksi meninggalkan halaman ujian!');
      }
    };
    ...
    const handleFullscreenChange = () => {
      if (!document.fullscreenElement) {
        handleViolation('fullscreen', '🚫 Dilarang keluar dari mode fullscreen!');
      }
    };
    ...
    const handlePaste = (e) => {
      e.preventDefault();
      handleViolation('paste', '🚫 Paste diblokir!');
    };

    // F. Right Click Prevention
    const handleContextMenu = (e) => {
      e.preventDefault();
      handleViolation('rightClick', '🚫 Klik kanan diblokir!');
    };

    // G. Drag Start (Mencegah drag gambar ke desktop/tab lain)
  ...
  // --- 5. KEYBOARD SECURITY (JANTUNG ANTI-CHEAT) ---
  useEffect(() => {
    if (!isActive) return;

    // Fungsi untuk menghapus clipboard
    const nukeClipboard = () => {
      try {
    ...
    const handleKeyDown = (e) => {
      // 1. DETEKSI PRINTSCREEN
      if (e.key === 'PrintScreen' || e.keyCode === 44) {
        e.preventDefault();
        nukeClipboard(); // Hapus clipboard langsung
        handleViolation('screenshot', '🚫 Screenshot dilarang! (PrintScreen detected)');
        return false;
      }

      // 2. DETEKSI TOMBOL WINDOWS (META) - Kunci Win+PrtScn
      // Kita tidak bisa stop Win+PrtScn, tapi kita bisa hukum user saat menekan tombol Win
      if (e.key === 'Meta' || e.key === 'OS' || e.keyCode === 91 || e.keyCode === 92) {
        e.preventDefault(); // Mencoba membatalkan event (seringkali OS override)
        handleViolation('systemKey', '🚫 Dilarang menekan tombol Sistem/Windows!');
        return false;
      }

      // 3. DETEKSI SHORTCUT LAIN (Alt+Tab, Ctrl+Shift+S, dll)
      if (
        e.key === 'F12' ||
    ...
    // Tambahkan KeyUp juga untuk menangkap "pelepasan" tombol screenshot
    const handleKeyUp = (e) => {
      if (e.key === 'PrintScreen' || e.keyCode === 44) {
        nukeClipboard();
        // Jangan trigger violation lagi disini biar tidak double count, cukup nuke clipboard
      }
    };
...
const UTBKStudentApp = () => {
  ...
  // State untuk SP1 (Surat Peringatan)
  ...
  const forceFullscreen = async () => {
    try {
      ...
      // Fallback alert jika gagal (misal di iOS Safari tertentu)
      // alert('⚠️ Gagal masuk fullscreen. Pastikan settings browser mengizinkan.');
    }
  };
  ...
  const handleSecurityViolation = (type, message, totalCount) => {
    ...
    // Force fullscreen kembali jika keluar (hanya jika bukan violation fatal)
    if (type === 'fullscreen') {
      forceFullscreen();
    }
  };

  // --- HANDLE SECURITY WARNING (SP1) ---
  const handleSecurityWarning = (type, message) => {
    setSp1Data({ type, message }); // Tampilkan Modal SP1
    // Blur focus agar tool screenshot tidak aktif jika memungkinkan
    if (document.activeElement) document.activeElement.blur();
  };

  const closeSP1 = () => {
    setSp1Data(null);
    forceFullscreen(); // Paksa masuk fullscreen lagi setelah tutup SP1
  };
        ...
        <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full relative text-center my-8">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6 text-left shadow-sm">
            <h3 className="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
              <AlertCircle size={16} className="text-indigo-600"/> Poin Penilaian (Metode IRT):
            </h3>
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex justify-between bg-green-50 px-2 py-1 rounded border border-green-100">
                <span className="flex gap-2 items-center">
                  <CheckCircle size={16} className="text-green-600"/>Bobot Soal
                </span>
                <span className="font-bold text-green-700">Dinamis</span>
              </li>
              <li className="text-xs text-gray-400 mt-2 italic">
                *Nilai ditentukan berdasarkan tingkat kesulitan soal dan tipe soal.
              </li>
            </ul>
          </div>
              ...
              <table className="min-w-full text-[10px] md:text-xs border-collapse">
                ...
                <tbody className="text-gray-900 bg-white font-medium">
                  {leaderboard.length === 0 ? (
                    <tr>
                      <td colSpan="18" className="p-6 text-center text-gray-500 italic">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="border border-gray-400 p-2 font-bold bg-teal-50 text-teal-900">
                          {row.score}
                        </td>
                      </tr>
                    );
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 select-none pb-10" style={{ userSelect: 'none', WebkitUserSelect: 'none' }}>
      ...
      {/* SECURITY COMPONENTS INTEGRATION (SOURCE 1) */}
      <AdvancedSecurityMonitor 
        onViolation={handleSecurityViolation}
        isActive={securityActive && !sp1Data} // Pause monitor saat modal SP1 muncul
        studentName={studentName}
        tokenCode={currentTokenCode}
        onForceSubmit={forceSubmitExam}
        onWarning={handleSecurityWarning} // Handler SP1
      />
