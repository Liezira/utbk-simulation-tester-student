...
const UTBKStudentApp = () => {
  ...
  if (screen === 'result') {
    ...
    // --- GLOBAL TIME ANALYSIS LOGIC ---
    const totalAllocatedMinutes = SUBTESTS.reduce((acc, curr) => acc + curr.time, 0);
    const totalAllocatedSeconds = totalAllocatedMinutes * 60;
    const timeUsedSeconds = totalAllocatedSeconds - timeLeft;
    const totalQuestions = SUBTESTS.reduce((acc, curr) => acc + curr.questions, 0);
    const avgTimePerQuestion = timeUsedSeconds / totalQuestions;
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8 flex justify-center items-center select-none overflow-y-auto">
        <div className="bg-white p-8 rounded-xl shadow-2xl max-w-4xl w-full text-center my-8">
          ...
          <div className="bg-indigo-50 border border-indigo-200 p-6 rounded-xl text-center mb-8 shadow-sm">
              <span className="text-sm text-gray-500 uppercase font-bold tracking-widest">Total Skor Akhir</span>
              <div className="text-7xl font-extrabold text-indigo-600 mt-2 mb-4">{totalScore}</div>
              
              <div className="flex justify-center items-center gap-4 text-sm border-t border-indigo-200 pt-4 mt-2">
                  <div className="bg-white px-3 py-1 rounded border border-indigo-100 flex items-center gap-2">
                      <Activity size={14} className="text-gray-400"/>
                      <span className="font-bold text-gray-600">Tempo: <span className={speedColor}>{speedAnalysis}</span></span>
                  </div>
                  <div className="bg-white px-3 py-1 rounded border border-indigo-100 flex items-center gap-2">
                      <Clock size={14} className="text-gray-400"/>
                      <span className="font-bold text-gray-600">Sisa Waktu: {formatTime(timeLeft)}</span>
                  </div>
              </div>
          </div>
