...
import { Clock, Ticket, AlertCircle, CheckCircle, XCircle, AlertTriangle, Lock } from 'lucide-react';
...
const UTBKStudentApp = () => {
  ...
  useEffect(() => {
    ...
    // 2. Mencegah Shortcut Developer Tools & Copy Paste
    const handleKeyDown = (e) => {
      if (
        ...
        e.preventDefault();
        alert('⚠️ DILARANG: Screenshot atau Membuka Developer Tools!');
      }
    };

    // 3. Deteksi Pindah Tab (Visibility Change)
  ...
  const handleTokenLogin = async () => {
    ...
    try {
      ...
      console.error(error); alert('Terjadi kesalahan koneksi.');
    }
  };
  ...
  const startTest = async (bypass = false) => {
    ...
    try {
        ...
        console.log("Fullscreen ditolak atau tidak didukung browser");
    }
  ...
  // Timers Main
  useEffect(() => {
    if (screen === 'test' && !isViolation && timeLeft > 0) { 
  ...
  const resumeTest = async () => {
    try {
        ...
        setIsViolation(false); // Fallback jika fullscreen gagal
    }
  };

  // --- UI RENDER ---

  // 1. VIOLATION OVERLAY (LAYAR MERAH)
  if (isViolation && screen === 'test') {
    return (
        <div className="min-h-screen bg-red-600 flex flex-col items-center justify-center text-white p-8 z-50 fixed top-0 left-0 w-full h-full text-center">
            ...
            <h1 className="text-4xl font-bold mb-4">PELANGGARAN TERDETEKSI!</h1>
            <p className="text-xl mb-8 font-medium max-w-2xl">{violationMsg}</p>
            <button 
                onClick={resumeTest}
                className="bg-white text-red-600 px-8 py-4 rounded-xl font-bold text-xl hover:bg-gray-100 transition shadow-lg"
            >
        ...
        <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full relative overflow-hidden text-center">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6 text-left shadow-sm">
            <h3 className="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
              <AlertCircle size={16} className="text-indigo-600"/> Ketentuan Penilaian:
            </h3>
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex items-center justify-between bg-green-50 px-3 py-2 rounded border border-green-100">
                <div className="flex items-center gap-2"><CheckCircle size={16} className="text-green-600" /> <span>Jawaban Benar</span></div>
              ...
              <li className="flex items-center justify-between bg-red-50 px-3 py-2 rounded border border-red-100">
                <div className="flex items-center gap-2"><XCircle size={16} className="text-red-500" /> <span>Jawaban Salah</span></div>
              ...
              <li className="flex items-center justify-between bg-orange-50 px-3 py-2 rounded border border-orange-100">
                <div className="flex items-center gap-2"><AlertCircle size={16} className="text-orange-500" /> <span>Tidak Menjawab</span></div>
          ...
          <button onClick={handleTokenLogin} className="w-full bg-indigo-600 text-white py-3.5 rounded-xl font-bold text-base hover:bg-indigo-700 transition shadow-lg transform hover:-translate-y-1">
            Mulai Ujian Sekarang
          </button>
          
          <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-center gap-2 text-xs text-gray-400">
             <Lock size={12} /> Mode Layar Penuh & Anti-Cheat Aktif
          </div>
        </div>
      </div>
    );
  ...
  if (screen === 'break') {
    return (
      <div className="min-h-screen w-full bg-gradient-to-br from-indigo-50 to-white flex flex-col items-center justify-center p-4 select-none">
        ...
        <div className="text-center">
          <p className="text-sm text-gray-400 font-medium tracking-wide">SUBTEST BERIKUTNYA AKAN DIMULAI OTOMATIS...</p>
        </div>
      </div>
    );
  }
  ...
  if (screen === 'result') {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8 flex justify-center items-center select-none">
        <div className="bg-white p-8 rounded-xl shadow-2xl max-w-4xl w-full text-center">
          ...
          <div className="mb-8">
            <span className="text-sm text-gray-400 uppercase tracking-widest font-bold">Total Skor</span>
            <div className="text-7xl font-extrabold text-indigo-600 mt-2">{totalScore}</div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-left">
            <div className="space-y-3">
              {leftSubtests.map(sub => (
                <div key={sub.id} className="flex justify-between items-center p-4 bg-gray-50 border border-gray-100 rounded-lg">
                  <span className="text-gray-700 font-semibold text-sm">{sub.name}</span>
                  <span className={`font-bold text-lg ${scores[sub.id] >= 0 ? 'text-indigo-600' : 'text-red-500'}`}>{scores[sub.id]}</span>
                </div>
              ))}
            </div>
            <div className="space-y-3">
          ...
          <div className="border-t pt-6">
            <p className="text-gray-400 text-sm mb-6 flex items-center justify-center gap-2"><Clock size={16}/> Skor telah disimpan secara otomatis.</p>
            <button onClick={() => { 
                document.exitFullscreen().catch(() => {});
                setScreen('landing'); setInputToken(''); setStudentName(''); 
            }} className="w-full md:w-1/2 bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg">Selesai / Logout</button>
          </div>
        </div>
      </div>
    );
  ...
  // Tambahkan style "user-select-none" di div utama agar teks tidak bisa diblok
  return (
    <div className="min-h-screen w-full bg-gray-50 select-none">
      <div className="sticky top-0 z-40 bg-indigo-700 text-white p-4 shadow-lg">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-indigo-200">Soal {currentQuestion + 1} dari {currentSubtest.questions}</p>
          </div>
                  ...
                  return (
                    <button key={idx} onClick={() => setCurrentQuestion(idx)}
                      className={`w-10 h-10 rounded font-semibold transition-all ${
                        ...
                        answered ? (isDoubtful ? 'bg-yellow-400 text-white' : 'bg-green-500 text-white') : 
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-lg shadow-lg p-6 min-h-[500px]">
              <div className="mb-6">
                ...
                {currentQ?.image && <img src={currentQ.image} alt="Soal" className="max-w-full rounded-lg shadow mb-4" onContextMenu={e => e.preventDefault()} />}
              </div>
              ...
              <div className="space-y-3 mb-6">
                {['A', 'B', 'C', 'D', 'E'].map((letter, idx) => (
                  <button key={letter} onClick={() => handleAnswer(letter)}
                    ...
                    <span className={`flex-shrink-0 w-8 h-8 flex items-center justify-center font-bold rounded ${
                      answers[key] === letter ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700'
                    }`}>
                      {letter}
                    </span>
                    <span className="flex-1 text-gray-700">{currentQ?.options[idx]}</span>
                  </button>
                ))}
              </div>

              <div className="flex items-center gap-3 mb-6">
                <input type="checkbox" id="doubtful" checked={doubtful[key] || false} onChange={() => setDoubtful(p => ({ ...p, [key]: !p[key] }))} className="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500" />
                <label htmlFor="doubtful" className="text-gray-700 font-medium cursor-pointer select-none">Tandai Ragu-ragu</label>
              </div>

              <div className="flex gap-3">
                <button onClick={() => currentQuestion > 0 && setCurrentQuestion(currentQuestion - 1)} disabled={currentQuestion === 0} className="px-6 py-3 bg-gray-500 text-white rounded-lg font-semibold hover:bg-gray-600 disabled:bg-gray-300">Kembali</button>
                <button onClick={() => {
                  if (currentQuestion < currentSubtest.questions - 1) setCurrentQuestion(currentQuestion + 1);
                  else if (confirm('Yakin ingin lanjut ke subtest berikutnya?')) {
                    if (currentSubtestIndex < testOrder.length - 1) { 
                      setScreen('break'); 
                      setBreakTime(10); 
                    } else setScreen('result');
                  }
                }} className={`flex-1 px-6 py-3 text-white rounded-lg font-semibold transition ${currentQuestion === currentSubtest.questions - 1 ? 'bg-green-600 hover:bg-green-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
                  {currentQuestion === currentSubtest.questions - 1 ? 'Selesai Subtest' : 'Selanjutnya'}
                </button>
              </div>
            </div>
          </div>
