...
const UTBKStudentApp = () => {
  ...
  const handleViolationLogic = async (type, message) => {
    ...
    // 1. Tambah Counter Global
    const newCount = violationCount + 1;
    setViolationCount(newCount);

    // 2. Log ke Firebase (Background)
    if (currentTokenCode) {
        try {
  ...
  useEffect(() => {
    if (screen === 'result' && currentTokenCode) {
        ...
        // HENTIKAN SECURITY HANYA SAAT SUDAH DI RESULT
        setSecurityActive(false); 
        ...
        const finishExamProcess = async () => {
            ...
            try {
                ...
                await updateDoc(tokenRef, { 
                    ...
                    violation: violationReason || null, // Simpan alasan jika didiskualifikasi
                    answers: answers,
        ...
        if (globalStartTime) {
            finishExamProcess();
             ...
             // Re-login logic (Load leaderboard only)
  ...
  const handleTokenLogin = async () => {
    ...
    try {
      ...
      if (confirm(`Login sebagai ${data.studentName}?`)) {
        ...
        setSp1Data(null); 
        setViolationCount(0); // Reset violation count on new login

        // CRITICAL: Aktifkan Security SEBELUM pindah screen
  ...
  const startTest = (bypass = false) => {
    ...
    // Pastikan security ON (redundant safety)
    setSecurityActive(true);
    setScreen('test');
  };
        ...
        timerRef.current = setInterval(() => {
            ...
            if (delta <= 0) {
                ...
                if (currentSubtestIndex < testOrder.length - 1) { 
                  // CRITICAL: JANGAN matikan securityActive saat break!
                  setScreen('break'); 
                  setBreakTime(10); 
                } 
                else { 
                  // Hanya matikan saat ujian benar-benar selesai
                  setSecurityActive(false); 
                  setScreen('result'); 
                }
  ...
  useEffect(() => { 
      if (screen === 'break' && breakTime > 0) { const t = setTimeout(() => setBreakTime(breakTime - 1), 1000); return () => clearTimeout(t); } 
          ...
          setTimeLeft(durationSec);
          setScreen('test'); // Security tetap aktif
      } 
  }, [breakTime, screen]);
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < currentSubtest.questions - 1) { setCurrentQuestion(currentQuestion + 1); } 
      ...
      if (currentSubtestIndex < testOrder.length - 1) { 
        // Transition to Break: Security Tetap ON
        setScreen('break'); 
        setBreakTime(10); 
      } 
      else { 
  ...
  if (screen === 'break') {
    return (
      <div 
        ...
        onContextMenu={(e) => e.preventDefault()}
      >
        {/* MONITOR TETAP NYALA SAAT ISTIRAHAT */}
        <AdvancedSecurityMonitor 
            isActive={securityActive && !sp1Data} 
            onViolationDetected={handleViolationLogic} 
        />
